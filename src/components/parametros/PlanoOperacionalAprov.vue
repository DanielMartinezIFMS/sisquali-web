<template>
    <cr-panel>
            <cr-panel columns="4" class="m-5" labelTop>
                ...
                <input-combo label="Laboratório" @change="(lab)=>{this.ensaios = lab.ensaios;}" v-model="laboratorio" :collection="laboratorios"/>
                <input-combo label="Ensaio" @change="(ens)=>{this.amostras = ens.tipoAmostras;}" v-model="ensaio"  :collection="ensaios"/>
                ...
                <cr-panel columns="2" colspan="2" labelTop box>
                    ...
                    <input-date label="Data Inicial"/>
                    <input-date label="Data Final"/>
                </cr-panel>
...
export default {
    ...
    methods: {
        filtrar: async function(){
            this.lista=await this.$get(ctt.rest+'/planoOperacinal/listar');
        },
